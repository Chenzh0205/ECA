...
class SmallNet(nn.Module):
    def __init__(self):
        ...
        self.convs = nn.Sequential(
            builder.conv3x3(1, 2, first_layer=True),
            nn.ReLU(),
            builder.conv3x3(2, 4),
            nn.ReLU(),
        ...
        self.linear = nn.Sequential(
            builder.conv1x1(784, 24),
            nn.ReLU(),
            builder.conv1x1(24, 10),
        )

    def forward(self, x):
        out = x.view(x.size(0), 1, 28, 28)
        out = self.convs(out)
        out = out.view(out.size(0), 784, 1, 1)
        out = self.linear(out)
        return out.squeeze()
...
class Conv2(nn.Module):
    def __init__(self):
        ...
        builder = Builder()
        self.convs = nn.Sequential(
            builder.conv3x3(1, 128, first_layer=True),  # Change input channels to 1
            nn.ReLU(),
            nn.MaxPool2d((2, 2)),  # Max pooling after first conv layer

            builder.conv3x3(128, 256),  # Second conv layer
            nn.ReLU(),
            nn.MaxPool2d((2, 2)),  # Max pooling after second conv layer
        )

        self.linear = nn.Sequential(
            builder.conv1x1(256*7*7, 512),  # Adjust dimensions accordingly
            nn.ReLU(),
            builder.conv1x1(512, 10),  # Fully connected layer 2 (output layer)
        )

    def forward(self, x):
        out = self.convs(x)
        out = out.view(out.size(0), 256 * 7 * 7, 1, 1)  # Flatten the output for the fully connected layers
        out = self.linear(out)
        return out.squeeze()
...
class EMNISTNet(nn.Module):
    ...
    def __init__(self):
        ...
        builder = Builder()
        self.convs = nn.Sequential(
            builder.conv3x3(1, 64, first_layer=True),
            nn.ReLU(),
        ...
        self.linear = nn.Sequential(
            builder.conv1x1(12544*2, 256),
            nn.ReLU(),
            builder.conv1x1(256, 47),  # 只修改这里，从 10 改为 47
        )

    def forward(self, x):
        out = x.view(x.size(0), 1, 28, 28)
        out = self.convs(out)
        out = out.view(out.size(0), 128 * 14 * 14, 1, 1)
        out = self.linear(out)
        return out.squeeze()
